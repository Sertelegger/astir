...
describe("runHook", () => {
  it("normalizes + posts a PostToolUse event", async () => {
    ...
    const code = await runHook(JSON.stringify({ session_id: "s1", cwd: "/repo", hook_event_name: "PostToolUse", tool_name: "apply_patch", tool_input: { input: "*** Update File: /repo/a.ts\n" } }), d as any);
    expect(code).toBe(0);
